...
{ PCB[pid]->exeStatus = CPU.exeStatus;
  ...
  insert_termio (pid, PCB[pid]->sockfd, str, endIO);
...
void submit_process (fname, a_sockfd)
char *fname;
int a_sockfd;
{
  FILE *fprog;
...
{ int pid;
 ...
 // printf("pid from get_ready is %d\n", pid);
  if (pid != nullReady)
  { context_in (pid);
    CPU.exeStatus = eRun;
  ...
  { 
    ...
    //   (follow the code for running the regular process)
	  //init_idle_process();
	  printf("begin idle process======================================================\n");
	  PCB[idlePid]->exeStatus = eRun;
	  context_in(idlePid);
