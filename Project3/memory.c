...
{ 
  ...
  { if (Debug) printf ("Instruction content = %x, Data content %.2f\n",
                        Memory[maddr].mInstr, Memory[maddr].mData);
    return (mNormal);
  }
}
...
{ int maddr;
  ...
  //printf("PCB[pid] Mbase is %d \n", PCB[pid]->Mbase);//for Debugging
